-- Saving an authorized existing Project MR is a server-confirmed edit, not
-- another approval submission. Existing decisions remain immutable and the
-- granted, pre-arrangement approved state remains actionable for Procurement.
-- No historical request, line, decision, movement or commercial row is rewritten.
-- Rollback: revoke edit grants through the audited command and ship a corrective
-- function migration; retain every saved revision and audit event.
begin;

alter table public.v1_material_request_revision_snapshots
  drop constraint if exists v1_material_request_revision_snapshots_snapshot_reason_check;
alter table public.v1_material_request_revision_snapshots
...
begin
  ...
  v_is_approved_edit := v_request.state = 'approved_for_arrangement';
  ...
  set scope_id = v_scope_id,
      ...
      state = case when v_is_approved_edit then 'approved_for_arrangement'
      ...
      current_action_code = case when v_is_approved_edit
        then 'arrangement_required' else 'request_approval_required' end,
  ...
  perform public.v1_write_audit_event(
    ...
    jsonb_build_object(
      ...
      'state', case when v_is_approved_edit
        then 'approved_for_arrangement' else 'awaiting_request_approval' end
...
insert into public.v1_audit_event_catalogue(event_type, severity) values
  ('material_request_saved_after_approval', 'normal')

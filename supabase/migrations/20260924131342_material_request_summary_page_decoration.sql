-- Keep the existing authorized filters and metrics intact, while decorating only
-- the page returned to the client. No rows or schema objects are changed.
-- Rollback: restore the previous function definition from
-- 20260901220449_material_request_action_intelligence.sql.
  ...
  with readable as materialized (
    ...
    select page.*,
      (select count(*)::integer
       ...
       where line.request_id = page.id) as item_count,
      public.v1_material_request_work_assignment_projection(
        page.id
      ) as work_assignment,
      public.v1_material_request_change_summary(page.id) as change_summary
    from page
  )

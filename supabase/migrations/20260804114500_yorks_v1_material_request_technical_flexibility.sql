-- Yorks V1: keep compatibility with current MR editor technical payload.
--
-- The frontend now emits optional non-commercial technical keys:
--   size, model, equipment_tag, planning_model_tag, quantity_suggested.
-- Older strict validation rejected these keys and blocked saving/submitting MR
-- payloads. This migration aligns server validation with the active UI schema
-- while preserving payload hygiene.
begin;
...
create or replace function public.v1_material_request_normalized_technical_attributes(
  p_technical_attributes jsonb,
  p_source_kind text
)
returns jsonb
language sql
...
as $$
  select jsonb_strip_nulls(jsonb_build_object(
    'size', nullif(btrim(coalesce(p_technical_attributes ->> 'size', '')), ''),
    'model', nullif(btrim(coalesce(p_technical_attributes ->> 'model', '')), ''),
    'equipment_tag', nullif(
      btrim(coalesce(p_technical_attributes ->> 'equipment_tag', '')),
      ''
    ),
    'planning_model_tag', nullif(
      btrim(coalesce(p_technical_attributes ->> 'planning_model_tag', '')),
      ''
    ),
    'quantity_suggested', case
      when p_source_kind = 'boq' then
        nullif(lower(btrim(p_technical_attributes ->> 'quantity_suggested')), '')
      else null
    end
  ));
$$;
...
grant execute on function public.v1_material_request_normalized_technical_attributes(
  jsonb, text
) to authenticated;

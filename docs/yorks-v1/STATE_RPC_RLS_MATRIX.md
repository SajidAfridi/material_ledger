...
| `v1_create_boq_group` | Active project actor with `boq.manage_folders` | project/selected real scope/name | yes | one empty custom folder in the selected scope only; no sibling scope or rows copied; projection and audit returned |
| `v1_rename_boq_group` | Active project actor with `boq.manage_folders` | group/scope/expected version/name/reason | yes | update only the selected scope folder's visible name; preserve group/template identity, worksheet title, rows and downstream references; audit old/new name |
| `v1_archive_boq_group` / `v1_restore_boq_group` | Active project actor with `boq.manage_folders` | custom group/scope/expected version/reason | yes | soft archive or restore only that selected-scope custom folder; preserve worksheet/history and reject duplicate-name restoration; Workshop Materials remains non-archivable |
| `v1_list_boq_folder_management` | Active project actor with `boq.manage_folders` | project/selected real scope/include-archived | no | protected active/archived folder projections, stable type and server-derived action/blocker flags; no cross-scope mutation |

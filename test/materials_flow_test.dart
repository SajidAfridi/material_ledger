...
void main() {
  ...
  double reservedOf(String id) => container
      .read(materialsProvider)
      .firstWhere((m) => m.id == id)
      .reservedQty;
  double availableOf(String id) => container
      .read(materialsProvider)
      .firstWhere((m) => m.id == id)
      .availableQty;
  ...
  group('Reservation lifecycle (FR-094)', () {
    ...
    test(
      'dispatch decrements stock + frees reservation; receipt only records',
      () async {
    ...
    test(
      'partial dispatch leaves the remainder open (status = partial)',
      () async {
        final startQty = qtyOf('mat-001');
        final notifier = container.read(materialRequestsProvider.notifier);
        await notifier.addRequest(
    ...
    test(
      'receipt cannot exceed dispatched and keeps a back-order live',
      () async {
        final notifier = container.read(materialRequestsProvider.notifier);
        await notifier.addRequest(
        ...
        ); // 8 still outstanding → stays live
      },
    );
  ...
  group('Goods receipt + weighted-average cost (FR-090)', () {
    test('receiveStock increments on-hand and rolls unit cost', () async {
      ...
      expect(
        container.read(materialUnitCostProvider('mat-001')),
        closeTo((120 * 45 + 40 * 50) / 160, 0.001),
      );
    });
  });
  ...
  group('Closeout enforcement (FR-095)', () {
    test(
      'cannot complete a project with open requests; can once cleared',
      () async {
